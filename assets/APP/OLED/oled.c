...
//清空一片区域 只是更新显存不会刷新屏幕
void OLED_Clear_area(u8 a,u8 b)
{
	u8 x,y;
  for(x=0;x<a;x++)
	{
    ...
    uint16_t i;
      ...
      for(x=0;x<128;x++)
       ...
       for(i=0;i<127;i++)
					...
					if(mode==0)
					//45/4096是指将4096缩小到50 以便在画点
					Draw_Line_Buffer(i,64- ( ((wave_data[i]>>num_shift)-2200)*30.0/300.0) ,i+1 ,64-( ((wave_data[i+1]>>num_shift)-2200)*30.0/300.0 ) );  //64减是因为起始点应在左上角 但波形起点在右下角  
				   if(mode==1)
					Draw_Line_Buffer(i,64- ( ((wave_data[i]>>num_shift)-350)*30.0/4000.0) ,i+1 ,64-( ((wave_data[i+1]>>num_shift)-350)*30.0/4000.0 ) );  //64减是因为起始点应在左上角 但波形起点在右下角  
